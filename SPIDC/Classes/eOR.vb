...
#End Region
        ...
        Try
            ...
            Using _nSqlDr
                If _nSqlDr.HasRows Then
                    ...
                    Loop
                Else
                    Return getTDN_History(assessno)
                End If
            End Using
        Catch ex As Exception
    ...
    Private Shared Function getTDN_History(ByVal assessno As String) As String
        Dim result As String = Nothing
        Try
            Dim _Query As String = "select top 1 '''' + TDN + '''' as TDN from HistAssess_FrmNewBilling_A where assessno='" & assessno & "'"
            Dim _SqlCommand = New SqlCommand(_Query, cGlobalConnections._pSqlCxn_RPTIMS)
            Dim _nSqlDr As SqlDataReader = _SqlCommand.ExecuteReader
            Using _nSqlDr
                If _nSqlDr.HasRows Then
                    Do While _nSqlDr.Read
                        Return _nSqlDr("TDN").ToString
                    Loop
...
" WHERE Amount IS NOT NULL ORDER BY SortOrder"
    ...
    Public Shared Sub Insert_eOR(ByVal eorno As String, Optional ByRef ERR As String = Nothing, Optional ByRef qry As String = Nothing)
        Try
            ...
            If TAXTYPE = "BUSINESS PERMIT" Then
                ...
                " and  PAYFILE.ACCTNO = (SELECT distinct acctno from [" & cGlobalConnections._pSqlCxn_OAIMS.DataSource & "].[" & cGlobalConnections._pSqlCxn_OAIMS.Database & "].dbo.onlinepaymentrefno where MAXORNO = '" & eorno & "' and emailaddr='" & Get_EMAIL(eorno) & "'" &
  ...
  "  and  LEDGER.tdn = (SELECT distinct AFBB.TDN from [" & cGlobalConnections._pSqlCxn_RPTIMS.DataSource & "].[" & cGlobalConnections._pSqlCxn_RPTIMS.Database & "].dbo.Assess_FrmNewBilling_B AFBB" &
...
" ) as xxx group by Acctno, MainAcctCode, MainAcctDesc, SubAcctCode, SubAcctDesc, xsquence, xsquencepen order by MainAcctCode,SubAcctCode"
    ...
    Function ALL_IN_ONE(ByVal TransactionType As String, ByVal txnrefno As String, ByRef qry As String, ByRef err As String) As Boolean
        Try
            ...
            Dim _Query1 As String = Nothing
            Dim _Query2 As String = Nothing
            Dim _Query3 As String = Nothing
            ...
            cEOR_Posting._pGateWayRefNo = Process.GatewayRefNo
            If cEOR_Posting._IsWithExistingGen_OR = False Then


                _Query1 = "INSERT INTO Gen_Or (setup, bookno, or_no, or_date, payor, [address], PayCode, Form_Use, [User], srs, isPrintORPErmanent, paymentrefno, isweb, gateway, GatewayRefNo, dfrom) " & _
            ...
            _Query2 = " UPDATE Receipts SET OR_Used = ( " & _
            ...
            '      _SqlCommand2.ExecuteNonQuery()
                ...
                '----------------------------------
                _Query3 = " SELECT (Receipts.SRS + CONVERT(varchar(6), GETDATE(), 112)  + '-' + gen_or.OR_NO) AS eORNO, Receipts.SRS, gen_or.OR_NO, Receipts.Book_No, Receipts.[user], GEN_OR.PaymentRefno, GEN_OR.Gateway, GEN_OR.or_date " & _
                    " FROM Receipts " & _
            ...
            Else
                _Query3 = " SELECT (Receipts.SRS + CONVERT(varchar(6), GETDATE(), 112)  + '-' + gen_or.OR_NO) AS eORNO, Receipts.SRS, gen_or.OR_NO, Receipts.Book_No, Receipts.[user], GEN_OR.PaymentRefno, GEN_OR.Gateway, GEN_OR.or_date " & _
                 ...
                 " AND GEN_OR.or_no BETWEEN Receipts.Beg_OR AND Receipts.End_OR " & _
                 " WHERE Receipts.Form = 'EOR' AND Receipts.BackEnd = 0 " & _
                 " AND gen_or.GateWayRefNo = '" & Process.GatewayRefNo & "'"
