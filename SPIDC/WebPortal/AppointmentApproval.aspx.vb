...
Public Class AppointmentApproval
    ...
    Sub load_GridView()
        Try
            ...
            '_nClass.Get_AppointmentRequests(cmbOFFICEFilter.Value, cmbTypeFilter.Value, cmb_SortBy.Value, cmb_Order.Value)
    ...
    Sub Get_Office()
        Try
            '----------------------------------
            cmbOFFICEFilter.DataSourceID = Nothing
            Dim _nClass As New cDalAppointment
            _nClass._pSqlConnection = cGlobalConnections._pSqlCxn_OAIMS
            _nClass._pSubGetOfficeWithAppointment()

            Dim _nDataSet As New DataSet()
            _nDataSet = _nClass._pDataSet

            cmbOFFICEFilter.DataSource = _nDataSet
            cmbOFFICEFilter.DataTextField = "OFFICE"
            cmbOFFICEFilter.DataValueField = "OFFICE"
            cmbOFFICEFilter.DataBind()
            cmbOFFICEFilter.Items.Insert(0, New ListItem("ALL", "ALL"))
