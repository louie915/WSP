...
Public Class AppointmentSetup2
    ...
    Public Shared Function GetEvents() As IList
        ...
        _nQuery = "Select *, GETDATE()dnow,DAY(appdate)AppDay,MONTH(appdate)AppMonth,YEAR(appdate)AppYear  from AppointmentSlot WHERE OFFICE='" & Trim(Office) & "'  ORDER BY APPDATE"
        ...
        For Each row In _nDataTable.Rows
            If _nSqlDr.HasRows Then
                Do While _nSqlDr.Read()
                    Dim a As Date = _nSqlDr.Item("AppDate")
                    Dim b As Date = _nSqlDr.Item("dnow")

                    Dim d As String = _nSqlDr.Item("AppDay")
                    Dim m As String = _nSqlDr.Item("AppMonth")
                    If d < 10 Then d = 0 & d
                    If m < 10 Then m = 0 & m
                    Dim y As String = _nSqlDr.Item("AppYear")
