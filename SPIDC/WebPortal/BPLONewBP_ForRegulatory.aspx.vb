Public Class BPLONewBP_ForRegulatory
    ...
    Private Sub btnApprove_ServerClick(sender As Object, e As EventArgs) Handles btnApprove.ServerClick
        Dim _nClass As New cDalNewBP
        _nClass._pSqlConnection = cGlobalConnections._pSqlCxn_BPLTIMS
        _nClass._pSubUpdateAttachmentStatus(hdnAppID.Value, hdnCode.Value, hdnReqCode.Value, "Approved", txt_ImageRemarks.Value)
        reload_Modal()
        snackbar("green", "Action : Requirement Approved")
    End Sub

    Private Sub btnReject_ServerClick(sender As Object, e As EventArgs) Handles btnReject.ServerClick
        Dim _nClass As New cDalNewBP
        _nClass._pSqlConnection = cGlobalConnections._pSqlCxn_BPLTIMS
        _nClass._pSubUpdateAttachmentStatus(hdnAppID.Value, hdnCode.Value, hdnReqCode.Value, "Rejected", txt_ImageRemarks.Value)
        reload_Modal()
        snackbar("green", "Action : Requirement Rejected")
    End Sub
    ...
    Private Sub btnSubmit_ServerClick(sender As Object, e As EventArgs) Handles btnSubmit.ServerClick
        Try
            ...
            _nClass._pSqlConnection = cGlobalConnections._pSqlCxn_BPLTIMS
            _nClass._pSubGenerate_ClearanceNo(hdnCode.Value, Clerance_No)
            _nClass._pSubUpdate_Status(hdnAppID.Value, hdnCode.Value, txt_Status.Value, Clerance_No, txt_Remarks.Value)

            '-- GET BPLTAS ACCTNO using WEB APPLICATION_ID
            _nClass.Application_ID = hdnAppID.Value
            _nClass._pSubGetACCTNO(ACCTNO)
            ...
            If GridView3.Rows.Count > 0 Then
                ...
                Next
            End If
            divComplete.InnerHtml = Nothing
            Load_Regulatory()
            sendEmail(hdnAppID.Value)
            check_if_complete(hdnAppID.Value)
    ...
    Sub check_if_complete2(ByVal AppID As String)
        ...
        If complete = True Then
            ...
            lnkTempApprove.Style.Add("display", "none")
            lnkTempApprove.Attributes.Add("visbile", "false")
            lnkTempApprove.Attributes.Add("disabled", "true")
    ...
    Private Sub btnTempApprove_ServerClick(sender As Object, e As EventArgs) Handles btnTempApprove.ServerClick
        Try
            ...
            If GridView1.Rows.Count > 0 Then
                For Each row As GridViewRow In GridView1.Rows
                    switch = DirectCast(row.FindControl("lbl_REQCODE"), Label).Text
                    _nClass._pSqlConnection = cGlobalConnections._pSqlCxn_BPLTIMS
                    _nClass._pSubGenerate_ClearanceNo(switch, Clerance_No)
                    _nClass._pSubUpdate_Status(AppID, switch, "Temporary Approved", Clerance_No, txtTempApproveRemarks.Value)
                Next
            End If
